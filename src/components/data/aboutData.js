
import umap from '../../assets/umap.webp'
import swad from "../../assets/swad.webp"
import prezy from "../../assets/prezy.webp"
import my from "../../assets/my.webp"
import cf from "../../assets/cf.webp"
import school from "../../assets/school.webp"


const aboutData = [
  
  {
    year: 2025,
    
    img:my,
    data: [
      "Focused on growth—learning, experimenting, and setting the foundation for something bigger."
    ]
  },
  {
    year: 2024,
    img:swad,
    data: [
      "Started Swad Natural a jaggery food brand, start evrything from scratch",
      "Spent time on designing, learning, failing, and restarting—again and again."
    ]
  },
  {
    year: 2023,
    img:prezy,
    data: [
      "Joined B.Tech in IT with zero experience but endless curiosity.",
      "Launched Prezy, an art poster brand—learned SEO, Shopify",
    ]
  },
  {
    year: 2022,
    // img:cf,
    data: [
      "Indore chapter: Completed my MAAC Institue graphic design course.",
    ]
  },
  {
    year: 2021,
    // img:school,
    data: [
      "Completed my schooling in this year during the pandemic chaos.",
      "Moved to Indore after exams, not knowing what to do next— just to explore",
    ]
  },
  {
    year: 2020,
    
    data: [
      "First spark of curiosity when i started using Internet, spent time exploring and observing how digital things work.",
      "Started dreaming of building something of my own one day."
    ]
  },
  {
    year: 2004,
    img:umap,
    data: [
      "Born—the first curious mind in the family destined to build, create, and explore, Driven by Wisdom"
    ] 
  }
];


export default aboutData;